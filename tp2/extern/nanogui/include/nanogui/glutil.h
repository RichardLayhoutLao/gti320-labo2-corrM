...
public:
    ...
    template <typename Matrix> void uploadAttrib(const std::string &name, const Matrix &M, int version = -1) {
        ...
        uploadAttrib(name, (uint32_t) M.storage().size(), (int) M.rows(), compSize,
                     glType, integral, M.storage().data(), version);
    }
    ...
    /// Initialize a uniform parameter with a 4x4 matrix (float)
    void setUniformMatrix4v(const std::string &name, const float* buf, bool warn = true) {
        glUniformMatrix4fv(uniform(name, warn), 1, GL_FALSE, buf);
    }
