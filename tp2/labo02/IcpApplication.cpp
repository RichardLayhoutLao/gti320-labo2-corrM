...
namespace
{
   ...
   */
  static inline void copyAndPerturbPoints(const gti320::Points3d& _A, gti320::Points3d& _B)
    {
      static const double maxRot = M_PI / 8.0;
      static const double maxTra = 2.0;

      const gti320::Matrix3d R = gti320::makeRotation(rand(-maxRot, maxRot), rand(-maxRot, maxRot), rand(-maxRot, maxRot));
      gti320::Vector3d t;
      t(0) = rand(-maxTra, maxTra); t(1) = rand(-maxTra, maxTra); t(2) = rand(-maxTra, maxTra);
 ...
 */
IcpApplication::IcpApplication(const char* _objFolder) : nanogui::Screen(Eigen::Vector2i(1280, 720), "GTI320 Labo 02", true, false, 8, 8, 24, 8, 0, 4, 1), m_objFolder(_objFolder)
{
